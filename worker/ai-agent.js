...
// ============================================================================
async function sendLeadToTelegram(args, env) {
...
🌐 *المصدر:* مساعد إبداع — الصفحة الرئيسية
🕐 *الوقت:* ${timeStr}`;
...
// ============================================================================
async function relayUpstream(upstream, writable, env, ctx) {
        ...
        if (finish === 'tool_calls' && !leadHandled) {
          ...
          for (const tc of toolCallsByIndex.values()) {
            ...
            try {
              ...
              leadResult = await sendLeadToTelegram(args, env);
            } catch (err) {
...
export default {
  async fetch(request, env, ctx) {
    ...
    // ابنِ الطلب الـ upstream — لاحظ: SYSTEM_PROMPT أولاً ليثبت كـ cache prefix
    const upstreamBody = {
      model: MODEL,
      stream: true,
      messages: [
    ...
    ctx.waitUntil(relayUpstream(upstream, writable, env, ctx));
