...
**الطريقة الموصى بها — سطر أوامر واحد:**

```bash
npx wrangler deploy --config worker/wrangler.ai-agent.toml
```

(أول مرة فقط: `npx wrangler login`)

ملف الإعداد يحمل اسم الـ Worker ومساره وربط الـ KV وتاريخ التوافق، فالنشر
ينسخ الملف كما هو في المستودع — لا لصق يدوي ولا احتمال أن ينحرف الإنتاج عن
`git`. الـ Secrets **لا تتأثر** بالنشر.

**الطريقة اليدوية (بديل):** اضغط `</> Edit code` → احذف الكود الافتراضي →
الصق محتوى `worker/ai-agent.js` بالكامل → `Deploy`.

> ⚠️ **درس من 2026-07-25:** عُدّل اسم النموذج داخل اللوحة الساعة 17:07Z، ثم
> أُضيف سطر `thinking: { type: 'disabled' }` في التزام الساعة 18:22Z ولم
> يُنشر قط. ظلّ الإنتاج بلا ذلك السطر أسابيع دون أي شيء يكشف الفارق. استخدم
> أمر `wrangler` أعلاه ولن يتكرر هذا.
